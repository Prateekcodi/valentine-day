...
export default function Day8Page() {
  ...
  const checkExisting = async function () {
    try {
      const pid = localStorage.getItem('playerId');
      console.log('📡 [Day8] Fetching Day status - room:', roomId, '| player:', pid);
  ...
  const fetchRoomInfo = async function () {
    try {
      console.log('📡 [Day8] Fetching room info:', roomId);
      const res = await fetch(API_URL + '/api/room/' + roomId);
      const data = await res.json();
      console.log('📡 [Day8] Room data received:', JSON.stringify(data));
      // setRoomInfo removed
      const pid = localStorage.getItem('playerId');
